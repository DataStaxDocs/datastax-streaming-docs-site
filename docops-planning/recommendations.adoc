...
=== Don't open xrefs in a new tab/window

Due to a known limitation in https://github.com/asciidoctor/asciidoctor/issues/4104[Asciidoctor] and https://gitlab.com/antora/antora/-/issues/808[Antora], specifying `^` or `window=_blank` in an xref will cause the xref to be calculated incorrectly, resulting in a 404.
