...
These cases are not ideal for a number of reasons, but simply put, kebab case makes URLs and file names easier to read and easier to type.
...
Also, in what scenarios does the copy button appear/not appear, and does the coppy button copy shell prompt characters?
====
...
[source, asciidoc]
----
ifeval::["{company}" == "nothing"]
*** xref:keyspace-concepts.adoc[Keyspace concepts]
endif::[]
----
...
[NOTE]
====
LLP: Need to check that redocly doesn't have any problems with `.yml` files.
====
...
I borrowed the following from https://github.com/couchbase/docs-site/blob/master/antora-playbook.yml[Couchbase's site playbook]:
